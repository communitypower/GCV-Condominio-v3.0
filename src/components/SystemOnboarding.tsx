...
type OnboardingResult = {
  ...
  syndic?: { id: string; email: string; name: string };
  invitation: { id: string; status: string; emailNormalized?: string };
      ...
      {result && (
        <section className="border border-emerald-500/25 bg-emerald-500/5 rounded-md p-5 space-y-3">
          ...
          <p className="text-sm text-zinc-300">
            {result.condominium.name} foi vinculado à conta {result.account.name}. O convite de{' '}
            {result.syndic?.email || result.invitation.emailNormalized || 'síndico responsável'} está {result.invitation.status}.
          </p>
